...
export default function TaskScheduler() {
  ...
  const scheduleTasksGreedy = (taskList: Task[]): Task[] => {
    ...
    const tasksWithDays = sortedTasks.map((task) => ({
      ...task,
      daysFromTomorrow: differenceInDays(task.deadline, tomorrow),
    }));

    const maxDeadline = Math.max(
      ...tasksWithDays.map((t) => t.daysFromTomorrow),
      0
    );

    const slots: (Task | null)[] = Array(maxDeadline + 1).fill(null);

    const scheduled: Task[] = [];

    tasksWithDays.forEach((task) => {
      for (let day = task.daysFromTomorrow; day >= 0; day--) {
        if (slots[day] === null) {
          slots[day] = task;
          scheduled.push({
            ...task,
            deadline: addDays(tomorrow, day),
          });
          break;
        }
      }
  ...
  const handleAddTask = () => {
    if (newTask.name && newTask.benefit > 0) {
      ...
      setNewTask({ name: "", deadline: addDays(new Date(), 1), benefit: 0 });
  ...
  return (
    <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-sm p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-semibold mb-2">Task Scheduler</h1>
          <p className="text-gray-500">
            Schedule from:{" "}
            {format(addDays(new Date(), 1), "EEEE, dd MMM", { locale: es })}
          </p>
        </div>
              ...
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setNewTask({
                      ...newTask,
                      benefit: parseInt(e.target.value),
                    })
                  }
                />
              </div>
      ...
      <div className="flex gap-4 mb-8 text-sm">
        <button className="text-blue-600 font-medium">
          Tasks{" "}
          <span className="ml-1 bg-blue-600 text-white px-2 py-0.5 rounded-full">
            {tasks.length}
          </span>
        </button>
      </div>
            ...
            <CardContent className="p-4">
              <div className="flex justify-between items-start mb-2">
                <div className="min-w-0 flex-1 mr-4">
                  <h3 className="text-lg mb-1 font-medium truncate">
                    {task.name}
                  </h3>
      ...
      {scheduledTasks.length > 0 && (
        <Card className="mt-6">
          <CardContent className="p-4">
           ...
           <footer className="mt-8 py-4 bg-gray-50">
        <div className="max-w-3xl mx-auto px-4 text-center text-xs text-gray-400">
          <p>Developed by:</p>
          <p>Isabella Sofía Arrieta Guardo • José Fernando González Ortiz • Daniel David Herrera Acevedo</p>
        </div>
      </footer>
    </div>
  );
